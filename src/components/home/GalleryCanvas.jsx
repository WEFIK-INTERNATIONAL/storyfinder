...
function galleryReducer(state, action) {
  switch (action.type) {
    case "SET_ZOOM":
      return { ...state, currentZoom: action.zoom, currentGap: action.gap };
    case "OPEN_IMAGE":
      return { ...state, isZoomActive: true, selectedIndex: action.index };
    case "CLOSE_IMAGE":
...
export default function GalleryCanvas() {
  ...
  const releaseFocusTrap = useCallback((container) => {
    if (focusTrapRef.current.handler) {
      container.removeEventListener("keydown", focusTrapRef.current.handler);
  ...
  const calculateBounds = useCallback((zoom, gap) => {
    ...
    const { scaledWidth, scaledHeight } = calculateGridDimensions(zoom, gap);
    const marginX = gap * zoom;
    const marginY = gap * zoom;
    return {
      minX:
        scaledWidth <= vw ? (vw - scaledWidth) / 2 : vw - scaledWidth - marginX,
  ...
  const initDraggable = useCallback(
    (zoom, gap) => {
      ...
      draggableRef.current = Draggable.create(canvasWrapperRef.current, {
        ...
        inertia: true,
        throwProps: {
          x: { velocity: "auto", resistance: 300, end: (v) => Math.round(v) },
          y: { velocity: "auto", resistance: 300, end: (v) => Math.round(v) },
        },
        onDragStart() {
    ...
    (targetZoom, newGap, animDuration = 1.2) => {
      ...
      if (newGap !== currentGap) {
        ...
        gsap.to(canvasWrapperRef.current, {
          duration: animDuration,
          width: GRID_CONFIG.cols * (GRID_CONFIG.itemSize + newGap) - newGap,
          height: GRID_CONFIG.rows * (GRID_CONFIG.itemSize + newGap) - newGap,
          ease: customEase,
        });
  ...
  const updateZoomButtons = useCallback((activeEl, zoomLevel) => {
    controlsRef.current?.querySelectorAll(".switch-button").forEach((btn) => {
  ...
  const exitZoomMode = useCallback(() => {
    ...
    if (!zs.isActive || zs.selectedIndex === null || !zs.scalingOverlay) return;
    ...
    document.removeEventListener("keydown", zoomStateRef.current._keyHandler);
    ...
    if (descriptionLinesRef.current?.length) {
      gsap.to(descriptionLinesRef.current, {
        ...
        onComplete: () => {
          ...
          gsap.set(descriptionLinesRef.current, { y: 80, opacity: 0 });
        },
      });
    }
  ...
  const playIntroAnimation = useCallback(() => {
    ...
    const centerX = (vw / 2 - matrix.m41) / matrix.a - GRID_CONFIG.itemSize / 2;
    const centerY = (vh / 2 - matrix.m42) / matrix.a - GRID_CONFIG.itemSize / 2;

    const els = GRID_ITEMS.map((_, i) => itemRefs.current[i]?.wrapper).filter(
      Boolean,
    );
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (idx !== null && entry.target === itemRefs.current[idx]?.wrapper)
            return;
          ...
          >
            {GRID_ITEMS.map((item, i) => (
              <div
                ...
                onClick={() => {
                  if (!zoomStateRef.current.isActive) enterZoomMode(i);
                }}
                onKeyDown={(e) => {
              ...
              >
                <Image
                  ...
                  sizes="320px"
                  quality={75}
                  loading={i < GRID_CONFIG.cols * 2 ? "eager" : "lazy"}
                  style={{
                    objectFit: "cover",
                    userSelect: "none",
                    pointerEvents: "none",
      ...
      >
        ...
        >
          {[
            ...
            { zoom: "fit", label: "Fit all images in view", text: "FIT" },
