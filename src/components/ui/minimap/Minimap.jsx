...
export default function Minimap({
  ...
  const startLoop = useCallback(() => {
    const tick = () => {
      ...
      if (Math.abs(st.currentTranslate - st.targetTranslate) > 0.1) {
        ...
        const snapped = indexToTranslate(translateToIndex(st.currentTranslate));
  ...
  const handleDragMove = useCallback(
    (e) => {
      ...
      if (!d.didDrag && Math.abs(pos - d.startPos) < DRAG_THRESHOLD) return;
      d.didDrag = true;
      e.preventDefault?.();
    ...
    [getPos, clamp, applyTransform, translateToIndex, syncOpacity, syncPreview]
  );
  ...
  useEffect(() => {
    ...
    stripEl.addEventListener('touchstart', handleDragStart, { passive: true });
    stripEl.addEventListener('touchmove', handleDragMove, { passive: false });
    stripEl.addEventListener('touchend', handleDragEnd, { passive: true });
    stripEl.addEventListener('touchcancel', handleDragEnd, { passive: true });
  ...
  return (
    <div className={`minimap-root ${className}`.trim()}>
      {/* ── Category label ── */}
      {category && (
        <div className="minimap-site-info" aria-label={`Category: ${category}`}>
          <p>Category</p>
          <p>
            <span>{category}</span>
          </p>
      ...
      >
        ...
        >
          ...
          <button
            ...
            onClick={handleShare}
            aria-label={
              copied ? 'Link copied to clipboard' : 'Share current image'
            }
            title="Share"
          >
            <svg
