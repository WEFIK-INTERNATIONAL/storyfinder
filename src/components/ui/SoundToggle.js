  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (!enabled && currentAmplitude < 0.01) {
        ...
        ctx.fillStyle = interpolateRgb(COLORS.primary, COLORS.mute, mute);
        for (let i = 0; i < W; i++) {
          const x = i - W / 2;
          const e = Math.exp((-x * x) / 50);
          const y =
            centerY +
            Math.cos(x * 0.4 - t * 8) * e * H * 0.35 * currentAmplitude;
          ctx.fillRect(i, Math.round(y), 1, 2);
        }
        ctx.fillStyle = interpolateRgb(COLORS.accent, COLORS.mute, mute);
        for (let i = 0; i < W; i++) {
          const x = i - W / 2;
          const e = Math.exp((-x * x) / 80);
          const y =
            centerY +
            Math.cos(x * 0.3 - t * 5) * e * H * 0.25 * currentAmplitude;
          ctx.fillRect(i, Math.round(y), 1, 2);
        }
      }
