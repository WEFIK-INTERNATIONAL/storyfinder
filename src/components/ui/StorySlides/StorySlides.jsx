...
export default function StorySlides() {
  ...
  useEffect(() => {
    changeStoryRef.current = (container) => {
      ...
      activeStoryRef.current = (activeStoryRef.current + 1) % stories.length;

      const story = stories[activeStoryRef.current];

      const profileNameElements = container.querySelectorAll('.profile-name p');
      ...
      const currentImgContainer = container.querySelector('.story-img .img');
