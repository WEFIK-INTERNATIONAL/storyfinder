...
const BlogList = ({ posts }) => {
    ...
    useGSAP(() => {
        ...
        if (featuredRef.current) {
            const imageWrapper = featuredRef.current.querySelector('.blog-featured-image-wrapper');
    ...
    const isFeaturedNew = posts?.[0]?.publishedAt && differenceInDays(new Date(), parseISO(posts[0].publishedAt)) <= 7;
            ...
            <header className="blog-header" ref={headerRef}>
                <div className="container">
                    ...
                    <p className="blog-subtitle" ref={subtitleRef}>
                        Essays, field notes, and thoughts on visual storytelling,
                ...
                <div className="container blog-content-wrapper">
                    ...
                    {featuredPost && (
                        <Link
                            ...
                            onClick={(e) => {
                                e.preventDefault();
                                if (pathname === `/blog/${featuredPost.slug}`) return;
                                navigateWithTransition(`/blog/${featuredPost.slug}`);
                            }}
                        ...
                        >
                            ...
                            <div className="blog-featured-content">
                                <div className="blog-card-meta">
                                    {isFeaturedNew && <span className="blog-badge-new">NEW</span>}
                                    <span className="blog-card-date">
                                        {featuredPost.publishedAt ? format(parseISO(featuredPost.publishedAt), 'MMM d, yyyy') : 'Recent'}
                                    </span>
                                    <span className="blog-spacer">•</span>
                                    <span className="blog-card-read-time">5 min read</span>
                                </div>
                                ...
                                <div className="blog-card-footer">
                                    <button 
                                        ...
                                        onClick={(e) => handleShare(e, `/blog/${featuredPost.slug}`, featuredPost.title)}
                            ...
                            {gridPosts.map((post, index) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            if (pathname === `/blog/${post.slug}`) return;
                                    ...
                                    >
                                        ...
                                        <div className="blog-card-content">
                                            ...
                                            <div className="blog-card-footer mt-auto pt-4">
                                                <div className="blog-card-meta">
                                                    {isNew && <span className="blog-badge-new">NEW</span>}
                                                    <span className="blog-card-date">
                                                        {post.publishedAt ? format(parseISO(post.publishedAt), 'MMM d, yyyy') : 'Recent'}
                                                    </span>
                                                </div>
                                                <button 
                                                    className="blog-share-btn" 
                                                    onClick={(e) => handleShare(e, `/blog/${post.slug}`, post.title)}
                                                    aria-label="Share post"
                                                >
                                                    <FiShare2 />
                                                </button>
